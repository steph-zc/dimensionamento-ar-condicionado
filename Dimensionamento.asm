...
.text
primeiro:
#Print das perguntas para obter os dados
	li $v0 4
	la $a0 first
	syscall
...
#Obter volume da sala
	mul.s $f0 $f1 $f2
	mul.s $f0 $f0 $f3
	
#Verificacao para todos os volumes da Tabela 1
	lwc1 $f1 trinta
	c.eq.s $f0 $f1 
	bc1t ptrinta
...
# Acoes no caso do volume estar presente na tabela 1
ptrinta:
	li $t2 1
...
segundo:
	# Print das perguntas para obter os dados sobre as janelas e incidencia de sol
	li $v0 4
	la $a0 third
	syscall
...
multijanela:
# Verificacao de situacao - Com Cortina ou Sem Cortina
	li $t2 1
	beq $t1 $t2 dmultia
	li $t2 2
	beq $t1 $t2 dmultib
	...
	dmultia:
	# Multiplicacao no caso Com Cortina - Reduzido a unidade - 160 - 212
	...
	dmultib:
	# Multiplicacao no caso Sem Cortina - Reduzido a unidade - 222 - 410
...
terceiro:
# Print pergunta para dados das portas - Area em metros quadrados
	li $v0 4
	la $a0 fifth
	syscall
	...
	syscall
	mov.s $f1 $f0
	# Multiplicacao referente a unidade - 125 por metro quadrado
	lwc1 $f2 centoevinteecinco
	mul.s $f8 $f1 $f2
	j quarto
		
quarto:
# Print pergunta para dados sobre pessoal - quantidade simultanea
	li $v0 4
	la $a0 sixth
	syscall
	...
	syscall
	mov.s $f1 $f0
	# Multiplicacao referente a unidade - 125 por pessoa
	lwc1 $f2 centoevinteecinco
	mul.s $f10 $f1 $f2
	j quinto
	
quinto:
# Print perguntas para dados sobre potencia de aparelhos presente na sala
	li $v0 4
	la $a0 seventh
	syscall
	...
	mul.s $f4 $f1 $f2 # Multiplicacao entre quantidade e media de potencia
	
	mul.s $f9 $f4 $f3 # Multiplicacao com o valor unitario - 0.9
	
	j fim
	
fim:
# Print dos valores em cada categoria
	li $v0 4
	la $a0 rum
	syscall
...
# Print para anunciar valor da categoria Total
	li $v0 4
	la $a0 rtotal
	syscall
	...
	# Print do valor da categoria total
	mov.s $f12 $f11
	li $v0 2
	syscall
	...
	# Print para anunciar valor em BTUs				
	li $v0 4
	la $a0 btu
	syscall
